...
export type ConsultationScenario = {
  ...
  caseTitle: string;
  situation: string[];
  riskCards: { title: string; level: string; text: string }[];
  debate: string;
  fileSlug: string;
};
...
<footer>Document produit dans le cadre d’un cas fictif de formation. Il ne constitue pas un conseil juridique. Base pédagogique : articles L. 4121-3 et R. 4121-2 du Code du travail.</footer></body></html>`;
  ...
  const facts = [['Effectif', scenario.workforce], ['Activité', scenario.activity], ['Objet', scenario.subject], ['Réunion', scenario.meeting]];
    ...
    <main>
      ...
      <section className="hero" id="top">
        <div className="hero-copy">
          <p className="eyebrow">Formation pratique · Atelier {scenario.workshopNumber}</p>
        ...
        <aside className="hero-card" aria-label="Aperçu de l’atelier">
          ...
          <p className="hero-card-number">{scenario.workshopNumber}</p><h2>{scenario.association}</h2><p>{scenario.workforce} · 35 à 45 min · Travail en groupe</p>
      ...
      <section className="section scenario" id="cas-pratique">
        ...
        <div className="scenario-grid">
          ...
          <aside className="legal-card"><p className="eyebrow">Repère pédagogique</p><h3>Pourquoi le CSE est-il consulté ?</h3>
            <p>Le CSE est consulté sur le DUERP et ses mises à jour. Dans une structure d’au moins 11 salariés, le document est mis à jour au moins chaque année.</p>
            <div className="legal-links"><a href="https://www.legifrance.gouv.fr/codes/article_lc/LEGIARTI000043893923" target="_blank" rel="noreferrer">Code du travail, art. L. 4121-3 ↗</a><a href="https://www.legifrance.gouv.fr/codes/article_lc/LEGIARTI000045386446" target="_blank" rel="noreferrer">Code du travail, art. R. 4121-2 ↗</a></div>
      ...
      <section className="section risks">
        ...
        <div className="risk-grid">{scenario.riskCards.map((risk, index) => <article className="risk-card" key={risk.title}><div className="risk-index">0{index + 1}</div><div><span className="risk-level">{risk.level}</span><h3>{risk.title}</h3><p>{risk.text}</p></div></article>)}</div>
      ...
      <section className="section workshop" id="atelier">
        ...
        <form onSubmit={downloadOpinion}>
          ...
          <fieldset className="form-section"><legend><span>2</span> Choisir le sens de l’avis</legend><div className="opinion-grid">
            <label className={`opinion-card positive ${form.opinion === 'favorable' ? 'selected' : ''}`}><input type="radio" name="opinion" value="favorable" checked={form.opinion === 'favorable'} onChange={() => update('opinion', 'favorable')} /><span className="opinion-icon" aria-hidden="true">+</span><strong>Avis favorable</strong><small>Le CSE approuve la mise à jour, avec ou sans réserves.</small></label>
            <label className={`opinion-card negative ${form.opinion === 'defavorable' ? 'selected' : ''}`}><input type="radio" name="opinion" value="defavorable" checked={form.opinion === 'defavorable'} onChange={() => update('opinion', 'defavorable')} /><span className="opinion-icon" aria-hidden="true">−</span><strong>Avis défavorable</strong><small>Le CSE estime l’évaluation ou les mesures insuffisantes.</small></label>
